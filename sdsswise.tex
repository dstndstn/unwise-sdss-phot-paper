...
\altaffiltext{3}%
{To whom correspondence should be addressed: dstn@cmu.edu}
\altaffiltext{4}%
...
would not be reported by the WISE pipeline and will not appear in the
...
case, the photometry task becomes linear: We know what each source
should look like in the WISE images, and we must compute the linear
sum of the sources that best matches the observed image.
...
For each WISE tile, we keep SDSS sources that are within the tile plus
